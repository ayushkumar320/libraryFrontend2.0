  ...
  return (
    <header className="sticky top-0 z-50 border-b border-gray-200 bg-white/80 backdrop-blur supports-[backdrop-filter]:bg-white/60">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-16 flex items-center">
        ...
        <nav className="hidden md:flex items-center gap-6 text-sm text-gray-700 ml-8">
          <button
            onClick={() => {
              onHomeClick?.();
              window.scrollTo({top: 0, behavior: "smooth"});
        ...
        <div className="ml-auto flex items-center gap-3">
          ...
          {/* Desktop auth actions only (hidden on mobile) */}
      ...
      {open && (
        <>
          ...
          <div
            id="mobile-menu"
            className="md:hidden relative border-t border-gray-200 bg-white z-50"
          >
            <div className="max-w-7xl mx-auto px-4 py-3 flex flex-col gap-2 text-gray-700">
              ...
              {/* Auth actions in hamburger only on mobile */}
              {!isAuthenticated ? (
                <button
                  onClick={() => {
                    onLoginClick?.();
                  ...
                  className="mt-2 inline-flex items-center gap-2 px-4 py-2 rounded-lg bg-blue-600 text-white hover:bg-blue-700 transition w-full justify-center"
                >
                  <LogIn className="w-4 h-4" /> Admin Login
                </button>
              ) : (
                <Link
                  to="/dashboard"
                  onClick={() => setOpen(false)}
                  className="mt-2 inline-flex items-center gap-2 px-4 py-2 rounded-lg bg-gray-900 text-white hover:bg-black transition w-full justify-center"
                >
                  <LayoutDashboard className="w-4 h-4" /> Dashboard
                </Link>
              )}
